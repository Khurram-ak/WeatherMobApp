...
class WeatherApi{
  ...
  Map<String,dynamic>? result;
    ...
    Future<Map<String,dynamic>> GetCurrent({Position? position,String? city})async{
      if(position!=null){
        var url = Uri.parse('http://api.weatherapi.com/v1/forecast.json?key=$ApiKey&q=${position.latitude},${position.longitude}&days=1');
        var response = await http.get(url);
        var decodedJson=jsonDecode(response.body);
        weatherApiModel=  WeatherApiModel.fromJson(decodedJson) ;
        print(weatherApiModel!.forecast!.forecastday![0].hour![0]);
        ...
        var url = Uri.parse('http://api.weatherapi.com/v1/forecast.json?key=$ApiKey&q=${city}&days=1');
        var response = await http.get(url);
        var decodedJson=jsonDecode(response.body);
        weatherApiModel=  WeatherApiModel.fromJson(decodedJson) ;
